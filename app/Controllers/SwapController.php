...
class SwapController
{
    ...
    public function cancel(array $params): void
    {
        ...
        if (!$swap || $swap['requester_id'] != Auth::id() || $swap['status'] !== SwapModel::STATUS_REQUESTED) {
            $this->jsonError('Cannot cancel this swap.', 403); return;
        }

        $ok = $this->swaps->decline((int)$params['id'], (int)$swap['provider_id']);
        if ($ok) $this->jsonSuccess(['message' => 'Request canceled. Credits returned to your wallet.']);
        else $this->jsonError('Unable to cancel swap.', 500);
    }
