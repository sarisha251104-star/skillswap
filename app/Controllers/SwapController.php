...
class SwapController
{
    ...
    public function cancel(array $params): void
    {
        ...
        if (!$swap || $swap['status'] !== SwapModel::STATUS_REQUESTED || (int)$swap['requester_id'] !== Auth::id()) {
            $this->jsonError('Cannot cancel this swap.',403);
            return;
        }

        $ok = $this->swaps->cancel((int)$params['id'], Auth::id());
        $ok ? $this->jsonSuccess(['message'=>'Swap cancelled. Credits returned to your wallet.'])
    ...
    public function review(array $params): void
    {
        ...
        $revieweeId = (Auth::id() === (int)$swap['requester_id']) ? (int)$swap['provider_id'] : (int)$swap['requester_id'];
